...
class OpenVINOTextDecoder(torch.nn.Module):
    ...
    def __init__(self, core: Core, model_path, device: str = "CPU"):
        super().__init__()
        self._core = core
        self.model = core.read_model(model_path)
    ...
    def forward(self, x: torch.Tensor, xa: torch.Tensor, kv_cache = None):
        """
...
@torch.no_grad()
def decode(model: "Whisper", mel: torch.Tensor, options: DecodingOptions = DecodingOptions()):
    """
